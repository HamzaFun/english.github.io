...
public class MessageDaoImp implements MessageDao {
    ...
    private static final String SQL_INSERT 		= "INSERT INTO `message` ( email, nomPrenom, adresseEmail, titre, message, date) VALUES ( ?, ?, ?, ?, ?, NOW())";
    ...
    public void create( Message message ) throws DAOException {
        ...
        try {
            ...
            preparedStatement = InitRP( connection, SQL_INSERT, true,
                    email, message.getNomPrenom(), message.getEmail(), message.getTitre(),
                    message.getMessage() );
            int status = preparedStatement.executeUpdate();
            if ( status == 0 ) {
                throw new DAOException( "échec de la création du message, aucune ligne ajoutee dans la table." );
            }
    ...
    public List<Message> list( UtilisateurDao utilisateurDao ) throws DAOException {
        ...
        try {
            connection = daoFactory.getConnection();
            preparedStatement = InitRP( connection, SQL_SELECT, false );
            ...
            fermer( rs, preparedStatement, connection );
        }
    ...
    public void delete( Long id ) throws DAOException {
        ...
        try {
            ...
            preparedStatement = InitRP( connection, SQL_DELETE, false, id );
            int status = preparedStatement.executeUpdate();
            if ( status == 0 ) {
    ...
    public void update( Message message, boolean vu ) throws DAOException {
        ...
        try {
            ...
            preparedStatement = InitRP( connection, SQL_UPDATE, false, message.isVu(), message.getId() );
            int status = preparedStatement.executeUpdate();
            if ( status == 0 ) {
                throw new DAOException( "échec de la mise a jour du théme, aucune ligne modifier." );
            }
        } catch ( SQLException e ) {
            throw new DAOException( e );
        } catch ( Exception e ) {
...
//        message.setUtilisateur( utilisateurDao.find( rs.getString( "email" ) ) );
